...
//
// function addDeviceToInput (inputId, address, callback) 
//   Adds the device at address to the input specified by inputId
//
Loggly.prototype.addDeviceToInput = function (inputId, address, name, callback) {
  ...
  var addOptions = {
    ...
    body: qs.stringify({ 'input_id': inputId, 'ip': address, 'name': name}).replace('"', '')
  };
...
//
// function removeDeviceFromInput (inputId, address, callback) 
//  Removes the device at address from input inputId
//
Loggly.prototype.removeDeviceFromInput = function (inputId, address, callback) {
  var addOptions = {
    uri: this.logglyUrl('devices', address, '?','input_id='+inputId),
    auth: this.config.auth,
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
...
//  Removes the device at address from all inputs
//
Loggly.prototype.removeDevice = function (address, callback) {
  var addOptions = {
    uri: this.logglyUrl('devices', address),
    auth: this.config.auth,
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
